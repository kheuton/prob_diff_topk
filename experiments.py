...
def training_loop_score_function_trick(model, loss_func, optimizer, num_epochs, train_dataset, val_dataset, negative_bpr_K,
                                       decision_func,
    ...
    for epoch in range(num_epochs):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            ...
            with tf.GradientTape() as jacobian_tape, tf.GradientTape() as loss_tape:
                ...
                model_loss =  loss_func(y_batch_train, expected_decisions_BS)

                
            # The lowercase "p" signifies that these are lists of length P, P = number of trainable variables
            jacobian_pMBS = jacobian_tape.jacobian(log_probs_MBS, model.trainable_weights)
            param_gradient_pBS = [score_function_trick(j, sample_decisions_MBS) for j in jacobian_pMBS]


            loss_gradients_BS = loss_tape.gradient(model_loss, expected_decisions_BS)
