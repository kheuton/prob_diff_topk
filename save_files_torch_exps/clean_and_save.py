...
def get_aransas_box_bounds(path):
    """
    ...
    aransas_df = pd.read_csv(f'{path}/raw-data/asurv_1950_to_2011/WHCR_Aerial_Observations_1950_2011.txt', encoding='latin1', sep='\t')
...
def points_to_boxes(gdf, study, temporal_res, box_length_m, keep_geometry_col, complete_idx_square, cut_water=True, path='code/prob_diff_topk/save_files_torch_exps', **kwargs):
    ...
    for tstep in np.sort(gdf[f'{DATE_NAME_TRANSLATOR[temporal_res]}_name'].unique()):
        ...
        with open(f"{path}/raw-data/boxes_total_bounds.txt", "r") as file:
            bounds = file.read()
    ...
    if cut_water:
        # set how to cut up the dataframe

        # y_left_lower_line = kwargs['y_left_lower_line']
        # y_right_lower_line = kwargs['y_right_lower_line']
        # y_left_upper_line = kwargs['y_left_upper_line']
        # y_right_upper_line = kwargs['y_right_upper_line']

        # combined_gdf = cut_gpd_water(combined_gdf, y_left_lower_line, y_right_lower_line)
        # combined_gdf = cut_gpd_water(combined_gdf, y_left_upper_line, y_right_upper_line, less=False)
        combined_gdf = cut_gpd_water(cut_gpd_water(combined_gdf, y1_up=0, y2_up=0.7, less=True), y1_up=0.35, y2_up=0.6, less=False)

    # if study == 'asurv':
    #     file_id = f"{2011 - years_through_2011 + 1}_to_2011_{temporal_res}_{box_length_m}M_small"
    #     filename = f"asurv_{file_id}"
    #     os.makedirs(f"asurv/{file_id}", exist_ok=True)
    #     EDA_file = f'asurv/{file_id}/{filename}_EDA.txt'

    # elif study == 'gps':
    #     file_id = f"2009_to_{2018 - years_cut_from_back}_{temporal_res}_{box_length_m}M_small"
...
def create_task(
    ...
    path='code/prob_diff_topk/save_files_torch_exps'):
    ...
    if 'save_files_torch_exps' in os.getcwd():
        ...
        save_path = 'prob_diff_topk/data_dir'
    else:
        save_path = 'code/prob_diff_topk/data_dir'

    if not os.path.exists(f'{save_path}/{study}/{timescale}_ctxtSize{context_size}_small'):
        os.makedirs(f'{save_path}/{study}/{timescale}_ctxtSize{context_size}_small')
    ...
    xtrain.to_csv(f'{save_path}/{study}/{timescale}_ctxtSize{context_size}_small/bird_train_x.csv')
    xval.to_csv(f'{save_path}/{study}/{timescale}_ctxtSize{context_size}_small/bird_valid_x.csv')
    xtest.to_csv(f'{save_path}/{study}/{timescale}_ctxtSize{context_size}_small/bird_test_x.csv')
    ...
    ytrain.to_csv(f'{save_path}/{study}/{timescale}_ctxtSize{context_size}_small/bird_train_y.csv')
    yval.to_csv(f'{save_path}/{study}/{timescale}_ctxtSize{context_size}_small/bird_valid_y.csv')
    ytest.to_csv(f'{save_path}/{study}/{timescale}_ctxtSize{context_size}_small/bird_test_y.csv')
